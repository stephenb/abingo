...
class Abingo
  ...
  def self.test(test_name, alternatives, options = {})
    ...
    if options[:multiple_participation] || !(participating_tests.include?(test_name))
      unless participating_tests.include?(test_name)
        ...
        Abingo.cache.add("Abingo::participating_tests::#{Abingo.identity}", participating_tests)
      end
  ...
  def self.retrieve_alternatives(test_name, alternatives)
    ...
    unless alternative_array = self.cache.get(cache_key) 
      alternative_array = self.parse_alternatives(alternatives)
      self.cache.add(cache_key, alternative_array)
    end
    alternative_array
  end
